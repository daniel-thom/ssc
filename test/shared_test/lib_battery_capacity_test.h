...
static void compareState(const capacity_state& cap_state, const capacity_state &s, const std::string& msg) {
    double tol = 0.01;
    EXPECT_NEAR(s.q0, cap_state.q0, tol) << msg;
    EXPECT_NEAR(s.qmax_thermal, cap_state.qmax_thermal, tol) << msg;
    EXPECT_NEAR(s.qmax, cap_state.qmax, tol) << msg;
    EXPECT_NEAR(s.I, cap_state.I, tol) << msg;
    EXPECT_NEAR(s.I_loss, cap_state.I_loss, tol) << msg;
    EXPECT_NEAR(s.SOC, cap_state.SOC, tol) << msg;
    EXPECT_NEAR(s.DOD, cap_state.DOD, tol) << msg;
    EXPECT_NEAR(s.charge_mode, cap_state.charge_mode, tol) << msg;
}
